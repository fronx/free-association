![](https://upload.wikimedia.org/wikipedia/commons/thumb/9/9f/Orca_pod_southern_residents.jpg/300px-Orca_pod_southern_residents.jpg)

# Introduction

This is a quick guide for programmers who want to use Transient. 

## Additional Resources
- [README](https://github.com/agocorona/transient/blob/master/README.md) - For a description of the internals for Haskell programmers
- [Transient Execution Model](https://github.com/transient-haskell/transient/wiki/Transient-execution-model.---How-to-get-things-done-with-the-transient-libraries) - Alternative tutorial on how to get things done with Transient libraries
...
The Haskell operator `<|>` is typically used to manage optional values (`Maybe` types). In Transient, it gains additional capabilities:

- Express parallel or distributed computations
- Compose widgets in web applications
- Combine console IO programs

```haskell
-- Example usage of <|> in Transient
main = keep $ do
    r <- async (return "hello") <|> async (return "world")
    liftIO $ print r
```

> **Note**: This doesn't change the fundamental nature of `<|>`. Instead, it adds appropriate behaviors for multi-threaded and distributed contexts.

## General-Purpose Design

Transient's general-purpose nature means it includes capabilities that might seem unusual to programmers from different domains:

- **Front-end web developers** may find it strange to have server-side capabilities
- **Server-side programmers** might be surprised by console input primitives and distributed computing
- **Distributed computing experts** may be unfamiliar with built-in map-reduce
- **Console application developers** might find multi-threaded console input unusual

However, modern applications increasingly require all these components. While a Facebook-scale application might need everything, most applications will use a subset of these features.

### Key Benefits

1. **Code Complexity Reduction**: Transient's unified approach simplifies complex systems
2. **Compact Code**: High-level abstractions lead to more concise implementations
3. **Executable Specifications**: The language serves as both specification and implementation
4. **Flexible Results**: Primitives can return zero, one, or many results as needed

## Combining Effects

Transient allows free combination of various effects:
- Multi-threading
...
Additional effects can be created as needed.

### Finalizations

Finalizations generalize exceptions for multi-threaded programs. Recent implementations include:
- Logging execution state to files
- Recovery of execution state from logs

### Language Design Philosophy

1. **General-Purpose**: Transient isn't domain-specific, allowing unrestricted effect usage
2. **Extensible**: Can be restricted to create domain-specific languages using standard Haskell techniques
3. **Practical Focus**: Emphasizes Haskell's strengths as an imperative language rather than pure functional programming

> "90% of computer engineering involves programming effects (excluding number crunching). Transient is designed for these practical problems - and number crunching too."
...
`input` is a Transient primitive for console IO. It uses a validation expression (`const True`), meaning that it accepts any string. You could read more about it later.
...
Now this is another story: this time `choose'` is a Transient primitive. It stops the current thread and launches a new thread for each of the elements in the list. Since the main thread dies, the computation could finish before the launched threads complete. This is the reason for the name "transient": because it manages transient threads. This is why `keep` is used. It prevents the program from exiting when the main thread dies. 
...
This can be expressed as  return 2 + return 2, since the trnasient monad is a instance of the class Number and the former is the definition of (+) in that class. Actually it can be expressed as 2+2, since the number instance convert any integer to the monad:

```haskell
result <- 2 + 2 :: TransIO Int
liftIO $ print result
```
Any binary operator can be constructed with this combination of applicative operators. This means that any combination of binary operators that can be created in pure code, can be executed within the transient monad without any change. The huge advantage is that each term could be executed in parallel. More on that later.
...
Let's see the difference of the former expression with this other expression:
...
```
After executing all the `onFinish` handlers, the execution resumes normally.
...
main= keep $ do
  ...
  return ()
```
The above program print the message:
    "hello"
    finish called, reason: "ok"
...
[this](https://gitter.im/Transient-Transient-Universe-HPlay/Lobby?at=5ef46626e0e5673398d33afb) has more information about backtracking.
